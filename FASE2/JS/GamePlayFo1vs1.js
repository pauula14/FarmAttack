class GamePlayFo1vs1 extends Phaser.Scene{
  constructor(){
      super("GamePlayFo1vs1");
  ...
  create(){
    ...
    // 1) BACKGROUND
    this.backgroundFo1 = this.add.image(0, 0, 'backgroundFo1vs1');
    this.backgroundFo1.setPosition(gameWidth/2, gameHeight/2);
    this.backgroundFo1.setDepth(1);

    // 2) PLAYER
    this.player1 = this.physics.add.sprite(150, 700, 'chicken1R').setScale(0.7).setDepth(2);
    this.player2 = this.physics.add.sprite(gameWidth-150, 700, 'chicken2L').setScale(0.7).setDepth(2);

    // 3) OBJETOS DE CONTROL DE FLUJO
    this.endTrigger1Empty = this.physics.add.sprite(35, 639, 'basketEmpty').setOrigin(0).setSize(100, 100).setDepth(2).setScale(1).refreshBody();
    this.endTrigger1Empty.body.setAllowGravity(false);
    this.endTrigger1 = this.physics.add.sprite(0, 620, 'basket1').setSize(100, 100).setOrigin(0).setDepth(2).refreshBody();
    this.endTrigger1.body.setAllowGravity(false);
    this.endTrigger1.setVisible(false);

    //
    this.endTrigger2Empty = this.physics.add.sprite(gameWidth-130, 640, 'basketEmpty').setOrigin(0).setSize(100, 102).setDepth(2).setScale(1).refreshBody();
    this.endTrigger2Empty.body.setAllowGravity(false);
    this.endTrigger2 = this.physics.add.sprite(gameWidth-155, 640, 'basket2').setSize(100, 100).setOrigin(0).setDepth(2).refreshBody();
    ...
    //Nivel 1
    this.ground.create(0, 601, 'platform').setOrigin(0,0).setScale(1.4,0.55).refreshBody().setVisible(false);//left
    this.ground.create(gameWidth/2+280, 601, 'platform').setOrigin(0,0).setScale(1.1,0.55).refreshBody().setVisible(false); //right

    //Nivel 2
    this.ground.create(440, 400, 'platform').setOrigin(0,0).setScale(1.35,0.5).refreshBody().setVisible(false);//center

    //Nivel 3
    this.ground.create(0, 210, 'platform').setOrigin(0,0).setScale(0.62,0.5).refreshBody().setVisible(false);//left
    this.ground.create(gameWidth/2+220, 210, 'platform').setOrigin(0,0).setScale(1.4,0.5).refreshBody().setVisible(false); //right


    //Grupo de huevos
    this.eggsP1 = this.physics.add.staticGroup();
    this.eggsP1.create(Phaser.Math.Between(gameWidth/3, gameWidth*2/3), 630, 'eggWhite').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP1.create(Phaser.Math.Between(0, 470), 500, 'eggWhite').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP1.create(Phaser.Math.Between(980, gameWidth-130), 500, 'eggWhite').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP1.create(Phaser.Math.Between(420, 900), 295, 'eggWhite').setOrigin(0,0).setScale(0.7).refreshBody().setDepth(2);
    this.eggsP1.create(Phaser.Math.Between(0, 170), 100, 'eggWhite').setOrigin(0,0).setScale(0.7).refreshBody().setDepth(2);
    this.eggsP1.create(Phaser.Math.Between(gameWidth/2+210, gameWidth-130), 100, 'eggWhite').setOrigin(0,0).setScale(0.7).refreshBody().setDepth(2);

//Phaser.Math.Between(0, 470)

    this.eggsP2 = this.physics.add.staticGroup();
    this.eggsP2.create(Phaser.Math.Between(gameWidth/3, gameWidth*2/3), 630, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP2.create(Phaser.Math.Between(0, 470),500, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP2.create(Phaser.Math.Between(980, gameWidth-130), 500, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP2.create(Phaser.Math.Between(420, 900), 295, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP2.create(Phaser.Math.Between(0, 170), 100, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
    this.eggsP2.create(Phaser.Math.Between(gameWidth/2+210, gameWidth-130), 100, 'egg').setOrigin(0,0).setScale(0.7).setDepth(2).refreshBody();
...
endArrived(player, end){
    ...
    if (this.playersArrived == 1){

      if (player == this.player1){
        console.log("gallina marron gano");
        winner = 1;
      }else{
        console.log("gallina blanka gano");
        winner = 2;
      }
  ...
  //FLUJO DEL JUEGO
    ...
    FinNivelFo1vs1(){
      ...
      this.scene.stop('GamePlayFo1vs1');
      this.scene.sendToBack('GamePlayFo1vs1');
      this.scene.start('Winner1vs1');
    }
